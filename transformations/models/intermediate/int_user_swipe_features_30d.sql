...
agg as (
    select
        ...
        coalesce(safe_divide(countif(liked and genero = '{{ genre_value }}'), countif(genero = '{{ genre_value }}')), 0.0)
            - coalesce(safe_divide(countif(liked), count(*)), 0.0) as preference_lift_genre_{{ feature_name }}_30d,
        {% endfor %}
        {% for feature_name, band_value in price_bands %}
        coalesce(safe_divide(countif(liked and lower(banda_precio) = '{{ band_value }}'), countif(lower(banda_precio) = '{{ band_value }}')), 0.0) as like_rate_price_band_{{ feature_name }}_30d,
        coalesce(safe_divide(countif(lower(banda_precio) = '{{ band_value }}'), count(*)), 0.0) as swipe_share_price_band_{{ feature_name }}_30d,
        coalesce(safe_divide(countif(liked and lower(banda_precio) = '{{ band_value }}'), countif(liked)), 0.0) as liked_share_price_band_{{ feature_name }}_30d,
